...
void IncrementalSegmenter<ClusteredPointT, PolicyName>::segment(
    ...
    SegmentedCloud& segmented_cloud, std::vector<Id>& cluster_ids_to_segment_ids,  // 初次使用没有内容，在addSegmentsToSegmentedCloud添加
  ...
  const size_t num_clusters = assignClusterIndices(partial_clusters);    //  cluster_id
  ...
  for (size_t i = 0u; i < partial_clusters.size(); i++) {
    ...
    if (old_segment_id == kInvId) {
      ...
      if (points_in_cluster > max_segment_size_) {
        ...
        point_indices.indices = getClusterIndices(partial_clusters, i);   // 获得points
        cluster_ids_to_segment_ids[cluster_id] = segmented_cloud.addSegment(    // 向segmented_cloud填充segment
