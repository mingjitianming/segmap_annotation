...
class DynamicVoxelGrid {
 ...
 private:
  ...
  struct VoxelData_ {
    Voxel_* old_voxel;
    typename IndexedPoints_::iterator points_begin;    // 一个voxel中点的起始迭代
...
DynamicVoxelGrid<_DVG_TEMPLATE_SPEC_>::removeIf(Func predicate) {
  ...
  // removed.
  while(v_read != v_end && !predicate(*(v_read->centroid)))        // predicate  判断体素点是否在圆柱区域内  false  在圆柱内 
    ++v_read;
  ...
  voxels_.erase(v_write, v_end);         //移除圆柱外侧的voxel
