...
void SegMatch::init(const SegMatchParams& params,
  ...
  descriptors_ = std::unique_ptr<Descriptors>(new Descriptors(params.descriptors_params));   // EigenvalueBased
  ...
  for (unsigned int i = 0u; i < num_tracks; ++i) {
    recognizers_.emplace_back(recognizer_factory.create());   // Simple Partitioned Incremental 三种方法可选
...
void SegMatch::processAndSetAsSourceCloud(
  ...
  descriptors_->describe(&segmented_source_clouds_[track_id]);        // 设置描述  "EnsembleShapeFunctions"  "EigenvalueBased"
...
const PairwiseMatches& SegMatch::recognize(const PairwiseMatches& predicted_matches,
  ...
  if (loop_closure != nullptr) {
    ...
    if (source_track_id != target_track_id) {           // TODO ZMY 多机器人建图
...
void SegMatch::update(const std::vector<laser_slam::Trajectory>& trajectories) {
  ...
  for (size_t i = 0u; i < trajectories.size(); ++i) {
    for (auto& pose: segmentation_poses_[i]){
      pose.second = trajectories.at(i).at(pose.first);         // 更新轨迹
    }
  }
