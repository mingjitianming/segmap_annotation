...
void EigenvalueBasedDescriptor::describe(const Segment& segment, Features* features) {      // segmentedCloud中的 segment
  ...
  const std::vector<size_t> col_indices_to_access = {0,1,2,1,2,2};    // 上三角索引
  ...
  if (diff_z < diff_x && diff_z < diff_y) {
    eigenvalue_feature.push_back(FeatureValue("pointing_up", 0.2));    // 确定上方的朝向是z
  } else {
