...
IncrementalGeometricConsistencyRecognizer::IncrementalGeometricConsistencyRecognizer(
  ...
  , max_consistency_distance_(max_model_radius * 2.0 + params.resolution)   // 50 * 2 +resolution
...
bool IncrementalGeometricConsistencyRecognizer::mustRemoveFromCache(
  ...
  pcl::Vector3fMapConst model_centroid = match.centroids_.first.getVector3fMap();   // source segment centroid
  pcl::Vector3fMapConst scene_centroid = match.centroids_.second.getVector3fMap();  // target segment centroid
