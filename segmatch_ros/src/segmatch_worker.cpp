...
void SegMatchWorker::init(ros::NodeHandle& nh, const SegMatchWorkerParams& params,
  ...
  export_run_service_ = nh.advertiseService("export_run",
                                            &SegMatchWorker::exportRunServiceCall, this);     // 是否有读取程序，在哪里????????
  segments_database_ = SegmentedCloud(false);                          // keep_only_last_view = false
  ...
  /***************************************************************************************************************/
   temp_export_map_ = nh.advertiseService(
    "temp_export_map", &SegMatchWorker::tempExportMap, this);
  /*****************************************************************************************************************/
...
bool SegMatchWorker::processLocalMap(
  ...
  if ((params_.localize && target_cloud_loaded_) || params_.close_loops) {
    ...
    // Process the source cloud.  进行分割并计算描述 eigenvalue,shape_feature;
    ...
    if (params_.close_loops) {
      ...
      if (recognized_matches.empty()) {
        segmatch_.transferSourceToTarget(track_id, latest_pose.time_ns);            // 回看 findMatches
      }
...
/**********************************************************************************************************/
bool SegMatchWorker::tempExportMap(segmatch_ros::TempExportMap::Request& req,
                                     segmatch_ros::TempExportMap::Response& res)
{
  PointICloud target_representation;
  segmatch_.getTargetRepresentation(&target_representation, compress_when_publishing_);
  translateCloud(Translation(0.0, 0.0, -params_.distance_to_lower_target_cloud_for_viz_m),
                 &target_representation);
   try {
    pcl::io::savePCDFileASCII(req.filename.data,
                         target_representation);
  }
...
void SegMatchWorker::publishMatches() const {
  ...
  publishLineSet(point_pairs, params_.world_frame, params_.line_scale_matches,      // 发布可视化匹配线
...
void SegMatchWorker::publishLoopClosures() const {     // 闭环可视化
  PointPairs point_pairs;
