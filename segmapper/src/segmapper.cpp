...
SegMapper::SegMapper(ros::NodeHandle& n) : nh_(n) {
  ...
  for (unsigned int i = 0u; i < params_.number_of_robots; ++i) {
    ...
    local_maps_.emplace_back(
        segmatch_worker_params_.segmatch_params.local_map_params, std::move(normal_estimator));  // local_map构建了PointsNeighborsProvider
...
void SegMapper::segMatchThread() {
  ...
  while (ros::ok()) {
    ...
    auto new_points = laser_slam_workers_[track_id]->getQueuedPoints();    // 获取LaserSlamWorker::scanCallback 中的结果
    if (new_points.empty()) {
    ...
    if (segmatch_worker_params_.localize) {                                                 // 定位选项
      ...
      if (segmatch_worker_.processLocalMap(local_maps_[track_id], current_pose,
        ...
        segmatch_worker_.update(updated_trajectories);    // 根据更新后的轨迹 对 segment_cloud,match等进行更新
  ...
  try {
    ...
    pcl::io::savePCDFileASCII(request.filename.data,
                          //    local_maps_.front().getFilteredPoints());
                              mapPoint2PointCloud(local_maps));
